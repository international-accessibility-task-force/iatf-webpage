<script lang="ts">
	import Title from '$lib/components/Title.svelte';
	import Meta from '$lib/components/Meta.svelte';
	import Header from '$lib/components/Header.svelte';
</script>

<svelte:head>
	<Title title="Manifesto" />
	<Meta
		content="Discover the International Accessibility Task Force Manifesto. Join our community dedicated to creating accessible technology for everyone."
	/>
</svelte:head>

<Header title="Manifesto" />

<main id="main">
	<section>
		<p>
			Welcome to the <b>International Accessibility Task Force (IATF)</b> a vibrant, inclusive community
			where assistive technology users, open-source enthusiasts, and advocates for accessibility unite.
			Our mission is to foster an environment where collaboration and innovation lead to the development
			of accessible solutions for all.
		</p>
		<h2>Our Community's Core</h2>
		<p>At the heart of IATF, we bring together</p>
		<ul>
			<li>
				Users of assistive technologies Those who rely on digital tools to navigate the web more
				easily.
			</li>
			<li>
				Open-source developers Visionaries who craft the building blocks of accessible technology.
			</li>
			<li>
				Advocates for accessibility and inclusion Champions who believe in creating an equitable
				digital world.
			</li>
		</ul>
		<p>
			Here, technology users and developers collaborate closely. Whether it's a request for a new
			tool or forming a workgroup for an open-source project, we work hand in hand to turn ideas
			into reality.
		</p>
		<h2>Our Guiding Principles</h2>

		Openness and Freedom: All creations must be licensed under the GPL License to guarantee free,
		universal access.
		<ol>
			<li>Empathy and Respect Treat yourself and others with kindness and understanding.</li>
			<li>Focus and Dedication Engage in one project at a time to ensure meaningful progress.</li>
			<li>
				Openness and Freedom All creations must be licensed under the GPL License to guarantee free,
				universal access.
			</li>
		</ol>
		<h2>Why These Principles Matter</h2>
		<ol>
			<li>
				Fostering Respect Every individual deserves dignity and respect, a principle that guides all
				interactions within our community.
			</li>
			<li>
				Ensuring Completion By focusing on one project at a time, we aim to see ideas through to
				fruition, making them available for the benefit of all.
			</li>
			<li>
				Promoting Openness The GPL License ensures our developments remain freely accessible,
				supporting a culture of shared innovation.
			</li>
		</ol>
		<p>
			Together, we're not just imagining a more accessible future; we're creating it. Join us in
			making a difference.
		</p>
		<p>With warm regards,<br /><b>The Admin Team</b> 🤗</p>
	</section>
</main>
